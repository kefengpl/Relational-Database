...
NestIndexJoinExecutor::NestIndexJoinExecutor(ExecutorContext *exec_ctx, const NestedIndexJoinPlanNode *plan,
    ...
    : AbstractExecutor(exec_ctx), plan_{plan}, child_executor_{std::move(child_executor)}, 
      ...
      left_tuple_{std::make_unique<Tuple>()}, left_tuple_dangling_{true} {
...
auto NestIndexJoinExecutor::Next(Tuple *tuple, RID *rid) -> bool { 
  ...
  while (true) {
    ...
    std::vector<RID> result{}; // 一般是只能取到一个 RID，根据实现逻辑，只有查找成功才会加入动态数组，查找失败则数组是空
    ...
    right_table_info_->table_->GetTuple(result[0], right_tuple.get(), exec_ctx_->GetTransaction()); // 获取唯一的右侧元组
    // 进行左右元组连接
...
void NestIndexJoinExecutor::MakeJoinTuple(std::unique_ptr<Tuple>& left_tuple, std::unique_ptr<Tuple>& right_tuple, Tuple *tuple) {
