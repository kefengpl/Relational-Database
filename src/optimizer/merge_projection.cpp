...
auto Optimizer::OptimizeMergeProjection(const AbstractPlanNodeRef &plan) -> AbstractPlanNodeRef {
  ...
  for (const auto &child : plan->GetChildren()) { // 一个 plan 可以有多层结点，它是一颗树，children 就是最上层算子的[子树，一个或者多个]
    children.emplace_back(OptimizeMergeProjection(child)); // 递归对子结点进行优化
  }
  // 提示：每个 plan 仅需知道它的 child 是谁即可。
  auto optimized_plan = plan->CloneWithChildren(std::move(children)); // 可以看作是克隆了该plan + 该plan 的后代(优化后的子树)
  ...
  if (optimized_plan->GetType() == PlanType::Projection) {
    ...
    // If the schema is the same (except column name)。下面的代码比较了 projection 孩子结点的 schema 和 projection 本身 schema 的异同
    ...
    if (std::equal(child_columns.begin(), child_columns.end(), projection_columns.begin(), projection_columns.end(),
      ...
      for (size_t idx = 0; idx < exprs.size(); idx++) { // 这个 for 循环获取了 #0.1 这样的列表达式，似乎在检验它是否是列表达式而不是(3+2)这种东西
