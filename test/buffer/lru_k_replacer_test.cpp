...
TEST(LRUKReplacerTest, StudentSampleTest) {
  ...
  for (int i = 0; i < 1000; i++) {
    record_access_task(i, true, true);
  }
  ASSERT_EQ(1000, lru_replacer.Size());

  // Remove frame id 250-500, set some keys to be non-evictable, and insert accesses
  ...
  for (int i = 0; i < 250; i++) {
    if (i < 150) {
      set_evictable_task(i, false);
    } else {
      set_evictable_task(i, false);
    }
  }

  // Ensure all items between 250, 500 have been removed.
  for (int i = 250; i < 500; i++) {
    remove_task(i);
  }
  ...
  for (int i = 500; i < 1000; i++) {
    if (i < 600) {
      evicted_elements.push_back(evict_task(true));
    } else if (i < 800) {
      evicted_elements.push_back(evict_task(true));
    } else {
      evicted_elements.push_back(evict_task(true));
