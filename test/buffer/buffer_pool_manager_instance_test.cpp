...
// NOLINTNEXTLINE
TEST(PageGuardTest, PageGuardReadTestRefactored) {
  ...
  // Scenario: When a page is fetched for reading, its pin count should increase.
  {
    auto reader_guard = bpm->FetchPageRead(page_id_temp);
    ASSERT_EQ(2, page0->GetPinCount());
  }
  ASSERT_EQ(1, page0->GetPinCount());

  // Scenario: Move construction of ReadPageGuard should transfer the ownership properly.
  {
    auto reader_guard = bpm->FetchPageRead(page_id_temp);
  ...
  // Scenario: Move assignment of ReadPageGuard should also transfer the ownership properly.
  {
    auto reader_guard_1 = bpm->FetchPageRead(page_id_temp);
    auto reader_guard_2 = bpm->FetchPageRead(page_id_temp);
    ASSERT_EQ(3, page0->GetPinCount());
...
TEST(PageGuardTest, ReadTest) {
  ...
  auto *disk_manager = new DiskManager(db_name);
  auto bpm = std::make_shared<BufferPoolManagerInstance>(buffer_pool_size, disk_manager, k);
...
TEST(BufferPoolManagerInstanceTest, IsDirty) {  // NOLINT
  ...
  ASSERT_EQ(1, bpm->UnpinPage(pageid0, false));  // 把这个测试进行了修改
