...
TEST(BPlusTreeTests, StudentTest) {
  // create KeyComparator and index schema
  auto key_schema = ParseCreateStatement("a bigint");
  GenericComparator<8> comparator(key_schema.get());
  ...
  std::vector<int64_t> keys{7, 14, 6, 8, 3, 11, 9, 17, 16, 4, 5, 10, 1, 12, 15, 13, 2};
  ...
  std::vector<int> shuffled_vec{9, 16, 6, 11, 2, 7, 1, 10, 5, 8, 3, 15, 12, 17, 4, 14, 13};
  for (auto key : shuffled_vec) {
    index_key.SetFromInteger(key);
    tree.Remove(index_key, transaction);
  }
...
TEST(BPlusTreeTests, InsertTest1) {
  ...
  // (wxx)  修改这里测试 ，原来是 10000
  int size = 10000;

  // std::vector<int64_t> keys{1, 14, 5, 4, 9, 2, 15, 10, 3, 11, 16, 17, 13, 7, 12, 6, 8};
  ...
  std::shuffle(keys.begin(), keys.end(), g);
  // std::vector<int> shuffled_vec{13, 2, 15, 9, 6, 16, 3, 11, 1, 10, 8, 12, 4, 5, 17, 14, 7};
