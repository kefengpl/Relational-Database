...
## Proj4 并发控制
### 任务一：锁管理器
**我们如何通过锁管理器保证读未提交、读已提交、可重复读这三种隔离级别？**
- ① 读未提交：两段锁协议(2PL)，它就可以造成脏读。2PL：扩张阶段：逐步获取你想要的锁；收缩阶段，在合适位置释放锁。
- ② 读已提交：在两段锁协议2PL的基础上，所有的X锁事务结束才能被释放，但是S锁可以在收缩阶段逐步释放。
- ③ 可重复读：实现SS2PL，也就是扩张阶段：逐步获取你想要的锁。这些锁只有在事务结束时才被释放。
...
- MySQL 的当前读：select lock in share mode, select for update ; update, insert ,delete。这些操作都会使用锁管理器根据事务的隔离级别进行加锁，是本次实验将
实现的部分。
